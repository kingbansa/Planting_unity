...
public class CactusSeedItemDatabase : MonoBehaviour {
    public Button cSeedButton;
    ...
    public void OnMouseDown()
    {
        if(state == "enable")
        {
            ...
            _confirmMenuCtrl.SetClickedItem(ConfirmMenuCtrl.ClickedItem.CactusSeed);
            state = "enable";
        }
    ...
    IEnumerator ItemEnable()
    {
        if (state == "enable" && !isDropBtn)
        {
            ...
            {
                if (probabilityVar <= 0.5f && probabilityVar > 0 && !plantName.Contains("BlowFishORCactus"))
                {
                    state = "disable";
                    cSeedButton.interactable = false;
                    btColor.color = new Color32(152, 152, 152, 255);
                    Debug.Log("아이템 대기시간");
                    int plantIDToInt = (int)PlantNameEnum.BlowFishORCactus_Lv1;
                    loginScript.Instance.UseItem("csItem", itemNum);
                    loginScript.Instance.ItemCountCheck("csItem");

                    loginScript.Instance.UpdatePlantListTable("BlowFishORCactus", plantIDToInt ,"WaterEXP", posRan, 1, 0.0f, true);
                    itemNumber = loginScript.csGetItem;
                    currentExp = loginScript.Exp;
                    //_plantExpPanel.CallExpList();
                    yield return new WaitForSeconds(3f);
                    state = "enable";
                    cSeedButton.interactable = true;
                    //이게 안먹히네*****************************************************
                    _cloudRecoTrackableEventHandler.CallRenewPlantList();
                    Debug.Log("복어아님 생성!");
                }
                else if (probabilityVar > 0.5f && !plantName.Contains("Weed"))
                {
                    state = "disable";
                    cSeedButton.interactable = false;
                    btColor.color = new Color32(152, 152, 152, 255);
                    Debug.Log("아이템 대기시간");
                    int plantIDToInt = (int)PlantNameEnum.Weed_Lv1;
                    loginScript.Instance.UseItem("csItem", itemNum);
                    loginScript.Instance.ItemCountCheck("csItem");

                    loginScript.Instance.UpdatePlantListTable("Weed", plantIDToInt, "WaterEXP", posRan, 1, 0.0f, true);
    ...
    private static CactusSeedItemDatabase instance;
    public static CactusSeedItemDatabase Instance
    {
        get
        {
            return instance;
    ...
    void Start()
    {
        Debug.Log("CactusSeeditemDatabase Start");
        loginScript.Instance.ItemCountCheck("csItem");
    ...
    void Update() // Update함수 이름 이렇게 바꾸고 ConfirmMenuCtrl에 있는 case ClickedItem.CactusSeed: 에서 호출함.
    {
        itemNumber = loginScript.csGetItem;
        currentExp = loginScript.Exp;
