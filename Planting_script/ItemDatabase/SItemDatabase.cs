...
public class SItemDatabase : MonoBehaviour {

    public GameObject sunshineObj;
    ...
    public GameObject sunAnim;
    public Transform sunAnimPlace;
    ...
    public void OnMouseDown()
    {
        if(state == "enable")
        {
            ...
            invActingMenuPanel.transform.position = sunshineObj.transform.position - UIPadding;
            _confirmMenuCtrl.SetClickedItem(ConfirmMenuCtrl.ClickedItem.SunPiece);
    ...
    private static SItemDatabase instance;
    public static SItemDatabase Instance
    {
        get
        {
            return instance;
    ...
    public void PlantLvUpLogic(int expName, float expAmount)
    {
        ...
        if (DBIndex >= 0)
        {
            if (plantLv[DBIndex] == 10 && sunExp[DBIndex] + expAmount >= sunLvUpExp[LvUpExpIndex])
            {
                //이경우 DB의 waterEXP = waterLvUpExp[LvUpExpIndex]
                loginScript.Instance.UpdatePlantExp(plantPosIndex[DBIndex], 10, expName, sunLvUpExp[LvUpExpIndex]);
            }
            if (plantLv[DBIndex] == 10 && sunExp[DBIndex] + expAmount <= sunLvUpExp[LvUpExpIndex])
            {
                //이경우 DB의 waterEXP = waterEXP + expAmount
                loginScript.Instance.UpdatePlantExp(plantPosIndex[DBIndex], 10, expName, sunExp[DBIndex] + expAmount);
            }
            if (plantLv[DBIndex] < 10 && sunExp[DBIndex] + expAmount >= sunLvUpExp[LvUpExpIndex] && fertilizerExp[DBIndex] >= fertilizerLvUpExp[LvUpExpIndex]
                && waterExp[DBIndex] >= waterLvUpExp[LvUpExpIndex])
            {
                ...
                loginScript.Instance.UpdatePlantExp(plantPosIndex[DBIndex], plantLv[DBIndex] + 1, expName, sunExp[DBIndex] - sunLvUpExp[LvUpExpIndex]);
            ...
            else if (plantLv[DBIndex] < 10)
            {
                ...
                loginScript.Instance.UpdatePlantExp(plantPosIndex[DBIndex], plantLv[DBIndex], expName, sunExp[DBIndex] + expAmount);
            }
        }
    }
    ...
    void Update() // Update함수 이름 이렇게 바꾸고 ConfirmMenuCtrl에 있는 case ClickedItem.SunPiece: 에서 호출함.
    {
        itemNumber = loginScript.sGetItem;
        currentExp = loginScript.Exp;
